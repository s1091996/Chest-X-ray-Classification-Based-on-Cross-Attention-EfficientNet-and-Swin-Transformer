...
EfficientNet-B5 baseline	Runnable	`main.py` currently creates this model with `timm.create_model(...)`.
Cross-attention fusion model	Experimental	Implemented in `model.py`, but is not yet wired into `main.py`'s training flow.
Keeping this distinction visible makes the repository reproducible and accurately represents the current implementation.
Dataset
The code expects the NIH ChestX-ray14 dataset and three split CSV files:
```text
<dataset root>/
├── images_001/images/
├── ...
...
python -m venv .venv
```
Activate the environment, install the appropriate PyTorch build for your system from pytorch.org, then install the remaining packages:
```bash
